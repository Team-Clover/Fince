...
export async function analyzeInvoiceText(ocrText) {
  ...
  try {
    ...
    console.warn(
      "Gemini analyzeInvoiceText failed, applying local fallback parser:",
      error.message,
    );
...
export async function analyzeInvoiceMultimodal(fileBuffer, mimeType) {
  try {
    ...
    const merchant = parsedData.merchant || "Unknown";
    let category = parsedData.category || "Operations";
    const keywordCat = applyKeywordCategorization(
      merchant,
      parsedData.items?.map((i) => i.name).join(" ") || "",
    );
    if (keywordCat) {
      category = keywordCat;
    }
    ...
    console.warn(
      "Gemini analyzeInvoiceMultimodal failed, will fall back to Tesseract + Local Parser in router.",
    );
...
export function chatWithContextLocally(
  userMessage,
  transactionsContext,
  budgetsContext,
) {
  ...
  if (query.includes("duplicate") || query.includes("double")) {
    ...
    const dupStr = duplicates
      .map(
        (t) =>
          `- **₹${t.amount}** from *${t.merchant}* on ${new Date(t.date).toLocaleDateString()} (Reason: Duplicate matches flagged)`,
      )
      .join("\n");
  ...
  if (
    ...
    const anomStr = anomalies
      .map(
        (t) =>
          `- **₹${t.amount}** from *${t.merchant}* in **${t.category}** (Reason: ${t.anomalyReason || "Spending spike"})`,
      )
      .join("\n");
  ...
  if (
    ...
    const budgetLines = budgets
      .map((b) => {
        const remainingText =
          b.remaining < 0
            ? `Exceeded by ₹${Math.abs(b.remaining)}`
...
export async function chatWithContext({
  ...
  try {
    ...
    const chat = model.startChat({
      history: [
        { role: "user", parts: [{ text: contextPrompt }] },
        {
          ...
          parts: [
            {
              text: "Understood. I am FINCE AI, your personal financial intelligence platform. How can I assist you today?",
            },
          ],
        },
        ...sdkHistory,
    ...
    const responseTextObj = response.response;
    return typeof responseTextObj.text === "function"
      ? await responseTextObj.text()
      : responseTextObj.text;
  } catch (error) {
    console.warn(
      "Gemini Chat assistant failed, using local rule-based response engine:",
      error.message,
    );
    return chatWithContextLocally(
      userMessage,
      transactionsContext,
      budgetsContext,
    );
...
export function generateFinancialReportLocally(
  transactionsContext,
  budgetsContext,
  period,
) {
  ...
  const sortedCats = Object.keys(catsMap).sort(
    (a, b) => catsMap[b] - catsMap[a],
  );
...
${
  budgets.length === 0
    ? "_No budget limits have been configured for this month._"
    : budgets
        .map((b) => {
...
export async function generateFinancialReport({
  ...
  try {
    ...
    console.warn(
      "Gemini generateFinancialReport failed, generating report locally:",
      error.message,
    );
    return generateFinancialReportLocally(
      transactionsContext,
      budgetsContext,
      period,
    );
...
export async function allocateBudgetWithAi({
  totalLimit,
  categoryPercentages,
  categoriesList,
}) {
  try {
    ...
    if (!data.allocations) {
      ...
      const hasFlatCategories = Object.keys(data).some((k) =>
        categoriesList.includes(k),
      );
      if (hasFlatCategories) {
        data = {
          allocations: data,
          rationale:
            "AI allocated category budgets matching historical shares.",
        };
      } else {
    ...
    console.warn(
      "Gemini allocateBudgetWithAi failed, running local allocation split:",
      error.message,
    );
    ...
    return {
      allocations,
      rationale:
        "Encountered Gemini quota limits or response parsing issue. Equal split (with a 10% safety buffer) has been configured across all categories.",
    };
  }
}

// Spend pattern auditor fallback
export async function generateSavingsAuditReport({
  currentMonthData,
  lastMonthData,
  username,
}) {
  try {
    ...
    console.warn(
      "Gemini generateSavingsAuditReport failed, running local comparison audit:",
      error.message,
    );

    const curTotal = Object.values(currentMonthData).reduce(
      (sum, c) => sum + c,
      0,
    );
    const lastTotal = Object.values(lastMonthData).reduce(
      (sum, c) => sum + c,
      0,
    );
