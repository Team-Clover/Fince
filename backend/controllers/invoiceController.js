...
export const uploadInvoice = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Tesseract OCR fallback for local text extraction
...
export const confirmInvoice = async (req, res) => {
  ...
  try {
    const invoice = await Invoice.findOne({ _id: invoiceId, $or: [{ userId }, { user: userId }] });
    if (!invoice) {
    ...
    const duplicateResult = await detectDuplicateInvoice(userId, invoice.extractedDetails, invoice.ocrText || invoice.extractedText);
    invoice.isDuplicate = duplicateResult.isDuplicate;
    invoice.duplicateOf = duplicateResult.duplicateOf;
...
export const getInvoices = async (req, res) => {
  ...
  try {
    ...
    if (user && user.familyCode) {
      ...
      invoiceQuery = { $or: [{ userId: { $in: familyUserIds } }, { user: { $in: familyUserIds } }] };
    }
...
export const deleteInvoice = async (req, res) => {
  ...
  try {
    ...
    await Transaction.deleteMany({ $or: [{ invoiceId }, { invoice: invoiceId }] });
...
export const manualInvoice = async (req, res) => {
  ...
  if (!merchant || !amount || !date || !category) {
    return res.status(400).json({ success: false, message: 'Merchant, amount, date and category are required.' });
  }
