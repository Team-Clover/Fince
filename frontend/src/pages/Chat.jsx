      ...
      if (res.ok) {
        ...
        if (history && history.length > 0) {
          ...
          setChats((prev) => {
            const existingChats =
              prev.length > 0
                ? prev.filter((c) => c.id !== "db-chat")
                : INITIAL_CHATS.filter((c) => c.id !== 1);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.aiResponse) {
          const aiText =
            data.aiResponse.content || data.aiResponse.message || "";
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Autonomous scenario simulation, tax auditing, and budgeting
              adviser
            </p>
          </div>
                  ...
                  <div className="h-full flex flex-col justify-center items-center text-center space-y-4 pt-20">
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-slate-800">
                        Ask FINCE AI anything
                      </h4>
                      <p className="text-xs text-slate-500 mt-1.5 leading-relaxed font-medium max-w-sm mx-auto">
                        "Analyze my latest grocery spends", "Am I on track with
                        my monthly budget limit?", or ask for savings
                        strategies.
                      </p>
                    </div>
                  </div>
                  ...
                  activeChat?.messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        {/* Avatar */}
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-[10px] font-bold border select-none ${
                            isAI
                        ...
                        {/* Message Bubble */}
                        <div
                          className={`p-4 rounded-2xl text-[13px] leading-relaxed shadow-sm text-left ${
                            isAI
                        ...
                        >
                          ...
                          <span
                            className={`text-[9px] block mt-2 font-medium opacity-70 ${isAI ? "text-gray-400" : "text-blue-100"}`}
                          >
                            {msg.time}
                          </span>
                        </div>
            ...
            <div className="p-4 bg-white border-t border-gray-100">
              <div className="max-w-4xl mx-auto">
                ...
                >
                  <button
                    type="button"
                    className="p-2.5 text-gray-400 hover:text-blue-500 transition-colors rounded-xl"
                  >
                    <FiPaperclip size={16} />
                  </button>
