...
const History = () => {
  ...
  const handleDelete = async (id, merchantName) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.update(toastId, { render: 'Server error. Could not delete.', type: 'error', isLoading: false, autoClose: 3000 });
      }
    } catch (err) {
  ...
  const processedInvoices = invoices
    .filter(inv => {
      ...
      const merchantMatch = merchant.toLowerCase().includes(search.toLowerCase());
      const filenameMatch = filename.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F8FAFC] text-slate-800 font-sans">
      ...
      <main className="flex-1 p-8 md:p-12 overflow-y-auto h-full relative">
        ...
        {/* Page Inner Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight font-outfit">Upload History</h1>
          <p className="text-slate-500 text-sm mt-1">Audit, search, and manage all your parsed invoices and receipts</p>
        </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100 text-xs text-slate-700">
                    {processedInvoices.length === 0 ? (
                      <tr>
                        <td colSpan="7" className="p-8 text-center text-slate-450 font-bold text-xs">
                          No matching invoices found in your archive.
                        </td>
                      </tr>
                    ) : (
                      processedInvoices.map((inv) => {
                        const fileName = inv.fileUrl ? inv.fileUrl.split('/').pop() : 'invoice.pdf';
                        return (
                          <tr key={inv._id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="p-4 font-outfit font-bold text-slate-800">{inv.merchantName || 'Unknown Vendor'}</td>
          ...
          <div className="bg-white max-w-2xl w-full p-6 border border-slate-200 rounded-2xl text-slate-800 space-y-6 max-h-[90vh] overflow-y-auto relative animate-scale-in shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-b border-slate-100 py-4 text-xs">
              <div className="space-y-1">
                <span className="text-slate-400 font-bold flex items-center gap-1"><FiTag className="w-3.5 h-3.5" /> Category</span>
                <span className="text-slate-850 font-extrabold text-sm">{selectedInvoice.category}</span>
              </div>
              <div className="space-y-1">
                <span className="text-slate-400 font-bold flex items-center gap-1"><FiCalendar className="w-3.5 h-3.5" /> Receipt Date</span>
                <span className="text-slate-850 font-extrabold text-sm">{new Date(selectedInvoice.invoiceDate || selectedInvoice.createdAt).toDateString()}</span>
              </div>
              <div className="space-y-1">
                <span className="text-slate-400 font-bold flex items-center gap-1">💸 Total Charged</span>
                <span className="text-blue-650 font-black text-sm font-mono">₹{(selectedInvoice.totalAmount || 0).toFixed(2)}</span>
              </div>
            </div>

            {/* Item list */}
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-slate-405 uppercase tracking-wider">Itemized Line Items</h4>
              <div className="bg-slate-50 border border-slate-150 rounded-xl overflow-hidden text-xs shadow-inner">
                ...
                <div className="divide-y divide-slate-150 max-h-36 overflow-y-auto">
                  {!selectedInvoice.purchasedItems || selectedInvoice.purchasedItems.length === 0 ? (
                    <div className="p-3 text-center text-slate-400 font-bold text-xs">No items parsed.</div>
                  ) : (
            ...
            {/* Raw OCR logs */}
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-slate-405 uppercase tracking-wider">Raw OCR Text Logs</h4>
              <pre className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-[10px] text-slate-700 font-mono overflow-auto max-h-32 whitespace-pre-wrap leading-relaxed select-all shadow-inner">
                {selectedInvoice.extractedText || 'No raw OCR logs associated with this parse.'}
              </pre>
            </div>
